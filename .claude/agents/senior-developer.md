...
> **작업 시작 전** `.claude/skills/senior-developer.md` 파일을 반드시 읽고 규약을 숙지한 후 작업을 시작한다.
...
**아키텍처 설계 / 공통 모듈 설계 / 기술 병목 해결 작업 완료 후, 반드시 설계 문서 파일을 작성한다.**

### 저장 위치
```
docs/design/YYYYMMDD-{주제-slug}.md
예) docs/design/20260226-grpc-service-pattern.md
```
...
## 컴포넌트 구조
[클래스/계층 구조 — 텍스트 다이어그램 또는 목록]

## 핵심 구현
[중요한 코드 패턴 또는 흐름 설명]

## 주니어 개발자 가이드
[이 패턴을 따를 때 주의할 점, 자주 실수하는 부분]

## 확장 포인트
[새 기능 추가 시 어디를 건드려야 하는가]
```

### 규칙
- 코드 리뷰 결과는 설계 문서 작성 **제외** (코드 리뷰 내용은 PR에 남김)
- 기술 병목 해결 시에는 `## 근본 원인` + `## 재발 방지` 섹션을 반드시 포함
- slug는 영문 소문자 + 하이픈 (예: `grpc-service-pattern`, `redis-session-structure`)

아래 기능 개발 요청에 대해 시니어 개발자 → 개발자(A 또는 B) → DBA 협업 → QA 순서로 전체 워크플로우를 진행해줘.
...
"코드나 텍스트가 길어질 경우, '계속 진행하시겠습니까?'라고 묻지 말고 가능한 한 번에 모든 내용을 출력해줘."
각 단계가 완료된 후 결과를 요약해서 다음 단계로 넘겨줘.


### Step 1. 시니어 개발자 — 설계
`senior-developer` 에이전트를 호출해서:
...
- gRPC 서비스 구현
- DB 작업이 필요한 경우 → Step 2-1 진행

#### Step 2-1. DBA 협업 (DB 작업이 있는 경우에만)
`dba` 에이전트를 호출해서:
